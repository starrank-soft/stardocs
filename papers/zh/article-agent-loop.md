...
视频创作很少在一次问答里结束。创作者会整理素材、生成图片和视频、搭出一个版本、播放检查，再继续调整镜头、节奏和文字。Agent 参与的是这段持续变化的过程：一次模型调用结束以后，素材仍在生成，用户仍在编辑，浏览器仍可能返回新的执行结果。最近 DeepSeek Harness 引发了很多关于 Agent Harness 的讨论；在此之前，StarCut 的 Video Agent Loop 已经围绕这些视频场景基本成型。我们也借这次讨论重新研究它能提供什么，但本文的主线仍然是 StarCut 如何设计和运行自己的 Agent Loop。

## 一、技术选型：为什么使用 Vercel AI SDK
...
DeepSeek Harness 也能覆盖模型与工具的通用循环，并进一步提供 Session、插件和上下文压缩等 Runtime 能力。它如果进入 StarCut，最适合先从这一层做替换实验；视频创作的外层运行机制仍要与现有项目和任务系统连接。

## 二、Video Agent Loop 的整体架构

StarCut 把整个 Agent Loop 分成两层。内循环处理当前这次能够连续完成的工作，外循环承接稍后才会发生的事实。
...
## 六、如何让 Codex、Claude Code 控制 StarCut
