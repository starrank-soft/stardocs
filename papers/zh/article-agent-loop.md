...
近期 DeepSeek Harness 的讨论让 Agent Harness 再次受到关注。在这之前，StarCut 的 Video Agent Loop 已经围绕媒体生成、项目协同和浏览器执行形成了一套实现。通用 Harness 可以处理模型调用、工具循环和上下文，视频创作还带来了异步媒体任务、持续变化的项目状态和多客户端执行；这些场景共同划定了 StarCut Agent Loop 的运行边界。
