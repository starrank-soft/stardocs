...
StarCut 对 Agent-native 创作环境的设计包含两个层次。首先，内置 Agent 要把读取项目、执行编辑、等待媒体生成和根据结果继续工作衔接起来；图片和视频可能在十几秒到几分钟后返回，其间用户仍会修改时间线，一次创作不能随着单次模型运行结束而中断。其次，同一份项目还要向 Codex、Claude Code 等外部 Agent 开放，使它们能够与内置 Agent 和创作者共同推进工程。Video Agent Loop 维持前一个过程，MCP 把同一套语义化编辑能力开放给外部 Agent；浏览器和服务端围绕同一份项目状态协作，为两个层次提供共同的运行基础。
