...
![Edit Video as Code 封面：虚拟文件视图与专业时间线连接到同一权威项目状态](../assets/edit-video-as-code/cover.png)
...
- `read` 时，从当前权威项目状态生成对应的文件内容；
- `edit` 时，把文件式操作反向解析成领域操作；
- 写入仍落到原来的结构状态、素材资源或项目元数据。
...
- **身份**：局部属性修改不应触发后代节点重建，已有引用和运行态身份需要继续成立；
- **移动**：Clip 从一条 Track 移到另一条 Track，改变的是归属和顺序，而不是“删除一段 XML，再插入一段相似 XML”；
- **写入范围**：全文替换依赖更大的旧文本匹配范围，语义操作只触及目标节点的属性、归属或顺序。底层协同机制仍负责合并，协议的价值是让写集合更接近用户意图。
...
单一稳定 ID 不会因为前面的 sibling 插入或重排而变化。文件路径负责限定文档，节点 ID 负责限定资源，因而不需要输出随层级增长的结构路径。
...
![RESTful-like 编辑协议在模型输出过程中逐步形成可验证的语义边界](../assets/edit-video-as-code/streaming-edit-flow.svg)
...
- 项目成为可持续发现和读取的外部记忆，不必完整复制进每一轮 Prompt；
- 读取成本随任务范围增长，局部任务只需展开相关文件或节点子树；
- 修改范围贴近用户意图，属性、归属、顺序和新子树各自保持清楚边界；
...
剪辑师真正担心的通常不是 AI 会不会执行 `split` 或 `move`，而是下面这些问题：

- **节奏感**：剪点是否落在动作、语言、音乐和情绪变化的合适相位，整段内容是否有呼吸、推进和释放；
- **美感**：构图、色彩、运动、声音和转场是否形成统一表达，而不是把一组局部正确的效果机械堆在一起；
- **网感**：是否理解特定平台、受众和内容类型的当下表达习惯，包括开头的吸引力、信息密度、字幕与音效节奏、梗的使用时机，以及什么时候应该打破模板。

“网感”不是一个神秘分数，也不等于无条件追逐热门模板。它更像经验丰富的创作者对平台语境和观众预期形成的动态先验：变化快、依赖场景，而且可能被一个反常规但更准确的创作选择推翻。
...
Agent 需要围绕候选剪点主动播放、seek、scrub 和比较局部结果；系统需要按时间组织运动、声音和语义证据；评价则应从“是否完成指令”推进到候选排序与人的偏好反馈。最终成片只能显示选择结果，而真实的编辑过程——看过什么、比较过什么、为何移动几帧——会提供更有价值的学习信号。

AI Cut 的下一步不只是把时间线排得更完整，而是获得选择性观察连续时间、形成候选、比较差异并用结果修正自己的能力。DSL 和编辑协议提供可执行的工程基础，感知、评价和反馈逐步形成剪辑判断。
