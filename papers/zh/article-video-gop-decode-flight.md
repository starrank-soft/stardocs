...
一张 4K RGBA 帧约 31.6 MiB，30 fps 接近每秒 1 GiB 的原始像素。只要这些帧穿过 IPC、落进 JavaScript 或 WASM 内存，再重新上传 GPU，前面获得的硬件解码优势很快就会被数据搬运抵消。这个弯路让高分辨率实时预览只剩下两个不可退让的原则：**硬件加速**与**零拷贝**。少一条，这条主链路就不成立。
...
GOP 没有一个适用于所有视频的固定上限，它由编码器配置、帧率、场景切换和交付方式共同决定。有的交付链路会主动缩短间隔，例如 [Apple HLS 的 I-frame 序列建议](https://developer.apple.com/documentation/http-live-streaming/hls-authoring-specification-for-apple-devices-appendixes/)每一到两秒提供一个关键帧；长 GOP 文件则很容易达到数百帧。[x264 的默认最大关键帧间隔](https://github.com/mirror/x264/blob/master/common/base.c)和 [x265 的默认最大帧内周期](https://x265.readthedocs.io/en/latest/cli.html#cmdoption-keyint)都是 250 帧，x265 甚至允许配置为无限 GOP。换成 30 fps，250 帧已经覆盖约 8.3 秒。因此，200 帧 GOP 并不是一个只存在于极端测试里的数字。
